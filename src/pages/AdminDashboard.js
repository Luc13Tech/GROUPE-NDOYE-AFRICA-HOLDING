...
import { VILLA_TYPES, DEFAULT_ADMIN_CONTENT, SITE, SERVICES, COMPANIES, LOTISSEMENT } from '../data/siteData';
...
function LineChart({ data, color='#c9a84c', height=90 }) {
  ...
  const w=400; const h=height; const pad=14;
  const pts=data.map((d,i)=>{ const x=pad+(i/(data.length-1))*(w-pad*2); const y=h-pad-((d.val-min)/range)*(h-pad*2); return[x,y]; });
  const path=pts.map((p,i)=>`${i===0?'M':'L'}${p[0]},${p[1]}`).join(' ');
  const area=path+` L${pts[pts.length-1][0]},${h-pad} L${pts[0][0]},${h-pad} Z`;
  const id=`gr${color.replace('#','')}`;
  return (<svg viewBox={`0 0 ${w} ${h}`} style={{width:'100%',height}} preserveAspectRatio="none">
    <defs><linearGradient id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.3"/><stop offset="100%" stopColor={color} stopOpacity="0.02"/></linearGradient></defs>
    <path d={area} fill={`url(#${id})`}/><path d={path} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    {pts.map((p,i)=><circle key={i} cx={p[0]} cy={p[1]} r="4" fill={color} stroke="var(--navy2)" strokeWidth="2"/>)}
  </svg>);
}
...
export default function AdminDashboard() {
  ...
  useEffect(()=>{
    intervalRef.current=setInterval(()=>{setChartData(prev=>{const last=prev[prev.length-1];return[...prev.slice(0,-1),{...last,val:last.val+Math.floor(Math.random()*4)+1}];});},8000);
      ...
      {modal&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&closeModal()}>
          <div className="modal-box" style={{width:'100%',maxWidth:'min(96vw,540px)'}}>
            ...
            {(modal==='addLot'||modal==='editLot')&&(<div>
            ...
            {(modal==='addProjet'||modal==='editProjet')&&(<div>
            ...
            {(modal==='addService'||modal==='editService')&&(<div>
              <FF label="Titre FR *" k="title_fr"/>
              <FF label="Titre EN" k="title_en"/>
            ...
            {(modal==='addPartner'||modal==='editPartner')&&(<div>
